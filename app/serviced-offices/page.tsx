...
export default function CoworkingSpaces() {
    ...
    useEffect(() => {
        const fetchWorkspaces = async () => {
            try {
                const response = await axios.get("https://api.anshulkumar.tech/api/workspace");
                if (response.data.success) {
                    const filteredWorkspaces = response.data.workspaces.filter(
                        (workspace) => workspace.workspaceType === "private-office"
                    );
                ...
                <div className="container px-4 mx-auto">
                    ...
                    <div className="relative z-10 py-12 mb-12 text-center">
                        ...
                        <h1 className="mb-4 text-4xl font-bold sm:text-5xl">
                            Serviced Offices Spaces
                        </h1>
                        <p className="mb-8 text-lg">
                            Discover the best Serviced Offices spaces that suit your business needs.
                        </p>
                    ...
                    <h2 className="font-sans text-3xl text-left">Suitable coworking office for you ❤️💕</h2>
                    {/* Grid of Office Cards */}
                    <div className="grid grid-cols-1 gap-8 py-12 sm:grid-cols-2 lg:grid-cols-3">
                        {filteredWorkspaces.map((workspace) => (
                            <Link key={workspace._id} href={`/serviced-offices/${workspace._id}`}>
                                <Card
                                    className="overflow-hidden transition-shadow duration-300 shadow-lg rounded-xl hover:shadow-2xl"
                                >
                                    <div className="relative w-full h-64">
                                        <Image
                                            src={`https://api.anshulkumar.tech${workspace.images[0] || "/placeholder.svg"}`}
